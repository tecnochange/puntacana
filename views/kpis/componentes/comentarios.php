        ...
        <table id="comentarios" class="table">
            <thead class="table-dark">
                <tr>
                    <th>Publicado por</th>
                    <th>Comentario</th>
                    <th class="text-center" style="min-width: 150px;">Frecuencia</th>
            ...
            <tbody>
                <?php
                $queryComentarios = mysqli_query($connect_kpis, "SELECT * FROM Comentarios_Kpis WHERE id_kpi = $idKpi ");
                $queryKPIS = mysqli_query($connect_kpis, "SELECT * FROM Kpis WHERE id = $idKpi");
                $dataKpis = mysqli_fetch_array($queryKPIS);
                ?>
                <?php while ($dataComentarios = mysqli_fetch_array($queryComentarios)): ?>
                    ...
                    <tr>
                        ...
                        <td class="text-center"><?= $dataComentarios["frecuencia"]; ?></td>
                        <td class="text-center"><?= $fechaInicio; ?></td>
                        <td class="text-center">
                            <?php if($VALIDAR_ROOT["editar"] || $dataRolKPI['nombre_rol'] == 'Lider KPI' ){ ?>
