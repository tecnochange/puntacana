...
//GUARDAR
if (isset($_POST["guardar_avance_kpi"])) {
    $ClassKpisCrud->actualizar_avance_Kpis($_POST);
    echo '<script> window.location = "?pg=kpis/consolidados/colaborador&id='.$_GET["id"].'#ref_' . $_POST["id_kpi"] . '"; </script>';
}
...
$array_kpis_general = $ClassKpisServicios->kpis_colaborador($user_log["id_empresa"], NULL, NULL, $id_colaborador);
$array_kpis_general_paginado = $ClassKpisServicios->kpis_colaborador_paginado($user_log["id_empresa"], NULL, NULL, $id_colaborador, $posicion, $longitud);
...
<div class="container-fluid">
    ...
    <div class="card mb-3">
        <div class="card-header">
            <h3>Ver KPIs de Mi Equipo para el año <?php echo $_SESSION["anio_fill"]; ?></h3>
        </div>
    </div>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    ...
                    for ($x = 0; $x < $total_pagina; $x++) {
                        if ($x == $_GET["p"]) {
                            echo '<li class="page-item active"><a class="page-link" href="?pg=kpis/consolidados/colaborador&id='.$_GET["id"].'&p=' . $x . '">' . ($x + 1) . '</a></li>';
                        } else {
                            echo '<li class="page-item"><a class="page-link" href="?pg=kpis/consolidados/colaborador&id='.$_GET["id"].'&p=' . $x . '">' . ($x + 1) . '</a></li>';
                        }
                    }
                    ?>
        ...
        <div class="accordion accordion-flush p-3" id="accordionVicepresidencias">
            ...
            <?php foreach ($array_kpis_general_paginado as $kpis) { ?>
                ...
                if ($kpis["frecuencia"] == "2") {
                    $meses = ["julio", "septiembre", "noviembre", "enero", "marzo", "mayo"];
                    $suma = 0;
                    $validos = 0;
                ...
                if ($kpis["frecuencia"] == "3") {
                    $meses = ["julio", "octubre", "enero", "abril"];
                    $suma = 0;
                    $validos = 0;
                ...
                <div class="accordion-item card mb-2 p-2">
                    <div class="card-body" style="padding: 5px 20px;" id="ref_<?= $kpis["id_kpi"] ?>">
                        <div class="row">
