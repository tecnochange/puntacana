...
if ($_POST["colaborador"] != "" && $_POST["descripcion"] != "") {
	...
	foreach ($_POST["colaborador"] as &$valor) {
		...
		mysqli_query($connect_clima, "INSERT INTO Publicaciones (id_user, id_empresa, id_proceso, 
			...
			'" . $valor . "', '" . utf8_encode($_POST["descripcion"]) . "', '3', '" . $_POST["visibilidad"] . "', '" . $_POST["copa"] . "', '" . $_POST["fecha_publicacion"] . "', '1', '" . $hoy . "'  ) ");
			...
			<div class="card-body">
				...
				<div style=" margin-top:15px; max-height:200px; overflow:auto">
					<table width="100%" style="color: #4286f5; font-size: 13px;">
						<tbody id="myTable">
							<?php
							$queryColaboradores = mysqli_query($connect_valentina, "SELECT * FROM Empleados WHERE id_empresa = '" . $_SESSION["id_empresa"] . "' ORDER BY nombre ASC ");
							while ($dataColaboradores = mysqli_fetch_array($queryColaboradores)) {
								echo '
						<tr>
						<td>
						&nbsp;&nbsp;&nbsp;&nbsp;' . $dataColaboradores["nombre"] . ' ' . $dataColaboradores["apellidos"] . '
				...
				<br>
				<select name="copa" required="required" class="form-control" style="margin-bottom:15px;">
				...
				<textarea name="descripcion" required="required" rows="4" class="form-control" rows="5" style="height: auto; margin-bottom:15px" placeholder="Ingresa una descripción detallada para el reconocimiento que desas enviar..."><?php echo $data["descripcion"] ?></textarea>
