...
$filtros = "";
if ($_POST["fill_nivel"]) {
    $filtros .= " AND id_nivel_jerarquico = '" . $_POST["fill_nivel"] . "' ";
}

$array_areas_Filtro = array();
$queryAreasFil = mysqli_query($connect_valoracion, "SELECT * FROM aa_sincronizacion 
WHERE anio = '" . $_SESSION["anio_ciclo"] . "' AND ciclo = '" . $_SESSION["ciclo"] . "' 
AND id_empresa = '" . $_SESSION["id_empresa"] . "'  GROUP BY id_area  ");
while ($dataAreasFill = mysqli_fetch_array($queryAreasFil)) {
    array_push($array_areas_Filtro, $dataAreasFill["id_area"]);
}

$array_niveles_Filtro = array();
$queryNivelFil = mysqli_query($connect_valoracion, "SELECT * FROM aa_sincronizacion 
WHERE anio = '" . $_SESSION["anio_ciclo"] . "' AND ciclo = '" . $_SESSION["ciclo"] . "' 
AND id_empresa = '" . $_SESSION["id_empresa"] . "'  GROUP BY id_nivel_jerarquico ");
while ($dataNivelFill = mysqli_fetch_array($queryNivelFil)) {
    array_push($array_niveles_Filtro, $dataNivelFill["id_nivel_jerarquico"]);
}
...
$array_niveles = array();
$queryNivel = mysqli_query($connect_valoracion, "SELECT * FROM aa_sincronizacion 
WHERE anio = '" . $_SESSION["anio_ciclo"] . "' AND ciclo = '" . $_SESSION["ciclo"] . "' 
AND id_empresa = '" . $_SESSION["id_empresa"] . "' " . $filtros . "  GROUP BY id_nivel_jerarquico ");
while ($dataNivel = mysqli_fetch_array($queryNivel)) {
    array_push($array_niveles, $dataNivel["id_nivel_jerarquico"]);
}
...
//DATOS GENERALES PARA LOS FILTRADOS
...
function RetornarColor($valor)
{
    ...
    $queryEI = mysqli_query($connect_valentina, "SELECT * FROM Escala_Medicion WHERE id_empresa = " . $_SESSION["id_empresa"] . "");
...
//DATOS PROCESADOS
$array_niveles_resumen = array();

foreach ($array_niveles as $nivel) {

    if ($nivel > 0) {

        $queryNivJer = mysqli_query($connect_admin, "SELECT * FROM Nivel_Jerarquico WHERE id = '" . $nivel . "'  ");
        $dataNivJer = mysqli_fetch_array($queryNivJer);

        $promedio_gerencia = 0;
        $counts_gerencia = 0;
        foreach ($array_evaluaciones_general as $evals) {
            if ($evals["id_nivel_jerarquico"] == $nivel) {
        ...
        $nodo = array(
            "nivel" => $dataNivJer["nombre"],
            "promedio" => round($promedio_gerencia, 2),
            "porcentaje" => round($porcentaje_gerencia, 2),
            "color" => $color_gerencia
        );

        array_push($array_niveles_resumen, $nodo);
    }
}

foreach ($array_niveles_resumen as $key => $row) {
    $aux[$key] = $row['promedio'];
}
array_multisort($aux, SORT_DESC, $array_niveles_resumen);
?>
        ...
        <div class="col-12">
            <div class="card mb-3">
                <div class="card-header">
                    <h3>
                        Informes Niveles Cargos <?= $dataCicloVal["anio"]; ?> | <small>Ciclo: <?php echo $dataCicloVal["nombre"]; ?></small>
            ...
            <div class="card">
                <div class="card-header">
                    <ul class="nav nav-pills justify-content-center">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="?pg=competencias/informes/niveles_cargo">Niveles Cargos </a>
                        </li>
                        <li class="nav-item">
...
<div class="container-fluid" style="max-width: 90%; margin: 0 auto;">
    <div class="row">
            ...
            <form action="" method="post">
                <div class="row">
                    ...
                    <div class="col-md-9 mb-3">
                        <select class="form-control multiples_responsables" name="fill_nivel">
                            <option value="">Filtrar por Nivel..</option>
                            <?php
                            foreach ($array_niveles_Filtro  as $nivel) {

                                $queryNivJer = mysqli_query($connect_admin, "SELECT * FROM Nivel_Jerarquico WHERE id = '" . $nivel . "'  ");
                                $dataNivJer = mysqli_fetch_assoc($queryNivJer);

                                if ($_POST["fill_nivel"] == $nivel) {
                                    echo '<option value="' . $nivel . '" selected >' . $dataNivJer["nombre"] . '</option>';
                                } else {
                                    echo '<option value="' . $nivel . '" >' . $dataNivJer["nombre"] . '</option>';
                                }
                            }
                            ?>
                        </select>
                ...
                <div class="card-body">
                    ...
                    <div style="width: 100%; background-color: #E9E9E9">
                        <div style="width: <?php echo $porcentaje_general; ?>%; background-color: <?php echo $color_general; ?>; text-align: center;font-weight: bold; padding: 5px; min-width: 60px;">
                            <?php echo round(($promedio_general_area * 100) / 5)  ?>%
                        </div>
                    </div>

                    <div style="margin-bottom: 20px">
                        Porcentaje nivel de desarrollo: <b><?php echo number_format(($promedio_general_area * 100) / 5);  ?>%</b><br>
                    </div>
                ...
                <div class="card-body" style="overflow-x: auto;">
                    <table width="100%">
                        <tr>
                            ...
                            $queryCompetencias = mysqli_query(
                                $connect_valoracion,
                                "SELECT *
                                FROM Competencias
                                WHERE id_empresa = '" . $_SESSION['id_empresa'] . "' AND anio = '" . $_SESSION['anio_ciclo'] . "' AND id_ciclo = '" . $_SESSION['ciclo'] . "' ORDER BY id DESC "
                            );
                            ...
                            foreach ($array_competencias as $competencia) {
                                ...
                                /*
                                if( $competencia["promedio"] >= 0 && $competencia["promedio"] <= 2.0 ){ $color_compt = "#FF7173" ; }
                                if( $competencia["promedio"] > 2.0 && $competencia["promedio"] <= 3.0 ){ $color_compt = "#FFC03A" ; }
                                if( $competencia["promedio"] > 3.0 && $competencia["promedio"] <= 3.75 ){ $color_compt = "#2ADAFF" ; }
                                if( $competencia["promedio"] > 3.75 && $competencia["promedio"] <= 4.50 ){ $color_compt = " #CDDC39" ; }
                                if( $competencia["promedio"] > 4.50 && $competencia["promedio"] <= 5 ){ $color_compt = "#8BC34A" ; }
                               */
        ...
        <div class="col-md-12">
            <div class="card fichas">
                <div class="card-header text-center">
                    <h2>Niveles de Cargos</h2>
                </div>
                <div class="card-body">

                    <?php
                    foreach ($array_niveles_resumen as $nivel) {
                    ?>

                        <div class="mt-1">
                            <b><?php echo $nivel["nivel"]; ?></b>
                        </div>
                        <div style="width: 100%; background-color: #E9E9E9">
                            <div style="width: <?php echo $nivel["porcentaje"]; ?>%; background-color: <?php echo $nivel["color"]; ?>; text-align: center;font-weight: bold; padding: 5px; min-width: 60px;">
                                <?php echo $nivel["porcentaje"]; ?>%
                            </div>
                        </div>
        ...
        <div class="col-md-12">
            <div class="card fichas">
                <div class="card-header">
                    <h2>Cargos Relacionados en este Informe</h2>
                </div>
                <div class="card-body">
                    <div align="left">
                        <?php
