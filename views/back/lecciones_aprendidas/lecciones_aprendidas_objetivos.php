...
if ($_SESSION["role_plataforma"] == 1) {
    ...
    $queryVPLecciones = mysqli_query($connect_clima, "SELECT DISTINCT(LA.id_vp) AS id_vp FROM Lecciones_Aprendidas AS LA INNER JOIN goforagile_admin.Vicepresidencia AS VP ON VP.id = LA.id_vp WHERE LA.id_empresa = '" . $_SESSION["id_empresa"] . "' AND LA.estado = 1 AND LA.anio = '" . $_SESSION["anio_fill"] . "' ORDER BY Areas.nombre");
...
if ($_SESSION["role_plataforma"] == 1) {
    $resultado = CargaLeccionesAdminObjEst($connect_clima, $connect_valentina, $filtro_la, $filtro_kr, $filtro_claves, $filtro_area, $filtro_vp);
} else if ($_SESSION["role_plataforma"] == 2) {
    $resultado = CargaLeccionesLiderObjEst($connect_clima, $connect_valentina, $filtro_la, $filtro_kr, $filtro_claves, $filtro_area, $filtro_vp, $_SESSION["id_user"]);
} else {
    $resultado = CargaLeccionesColaboradorObjEst($connect_clima, $connect_valentina, $filtro_la, $filtro_kr, $filtro_claves, $filtro_area, $filtro_vp, $_SESSION["id_user"]);
}
...
$querySM72 = mysqli_query($connect_valentina, "SELECT * FROM Submenu_Empresa WHERE id_empresa = " . $_SESSION["id_empresa"] . " AND estado = 1 AND id_menu = 7 AND id_submenu = 42");
            ...
            <div class="card-header" style="background-color: #FFFFFF !important;">
                <div class="row">
                    <div class="col-md-12" style="text-align: start !important;">
                        <h4><i class="fas fa-book" id="iconCabecera"></i>&nbsp;&nbsp;|&nbsp;&nbsp;<?php echo $dataSM72["nombre"]; ?></h4>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                while ($dataVP = mysqli_fetch_array($queryVP)) {
                                    ...
                                    if (mysqli_num_rows($queryLideres) > 0) {
                                        while ($dataLideres = mysqli_fetch_array($queryLideres)) {
                                            $queryEmple = mysqli_query($connect_valentina, "SELECT * FROM Empleados WHERE id = '" . $dataLideres["id_responsable"] . "' ");
