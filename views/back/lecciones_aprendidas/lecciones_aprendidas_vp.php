...
if ($_POST["vp_lA"] == "") {
    $_SESSION["vp_lA"] = "";
    $filtro_vp .= " AND id_vp IS NOT NULL AND id_vp <> ''";
}
...
if ($_SESSION["role_plataforma"] == 1) {
    $queryAreaLecciones = mysqli_query($connect_clima, "SELECT DISTINCT(LA.area) AS area FROM Lecciones_Aprendidas AS LA INNER JOIN goforagile_admin.Areas AS Areas ON Areas.id = LA.area WHERE LA.id_empresa = '" . $_SESSION["id_empresa"] . "' AND LA.estado = 1 AND LA.anio = '" . $_SESSION["anio_fill"] . "' ORDER BY Areas.nombre");
    $queryVPLecciones = mysqli_query($connect_clima, "SELECT DISTINCT(LA.id_vp) AS id_vp FROM Lecciones_Aprendidas AS LA INNER JOIN goforagile_admin.Vicepresidencia AS VP ON VP.id = LA.id_vp WHERE LA.id_empresa = '" . $_SESSION["id_empresa"] . "' AND LA.estado = 1 AND LA.anio = '" . $_SESSION["anio_fill"] . "' ORDER BY VP.nombre");
    $queryEstrategicoLecciones = mysqli_query($connect_clima, "SELECT DISTINCT(LA.id_okr_estrategico) AS estrategico FROM Lecciones_Aprendidas AS LA INNER JOIN goforagile_okrs.Objetivos_estrategicos AS OE ON OE.id = LA.id_okr_estrategico WHERE LA.id_empresa = '" . $_SESSION["id_empresa"] . "' AND LA.estado = 1 AND LA.anio = '" . $_SESSION["anio_fill"] . "' ORDER BY OE.objetivo");
    $queryOrganizacionalLecciones = mysqli_query($connect_clima, "SELECT DISTINCT(LA.id_okr_organizacional) AS organizacional FROM Lecciones_Aprendidas AS LA INNER JOIN goforagile_okrs.Okrs AS O ON O.id = LA.id_okr_organizacional WHERE LA.id_empresa = '" . $_SESSION["id_empresa"] . "' AND LA.estado = 1 AND LA.anio = '" . $_SESSION["anio_fill"] . "' ORDER BY O.objetivo_okr");
} else if ($_SESSION["role_plataforma"] == 2) {
    $queryAreaLecciones = mysqli_query($connect_clima, "SELECT DISTINCT(LA.area) AS area FROM Lecciones_Aprendidas AS LA INNER JOIN goforagile_admin.Areas AS Areas ON Areas.id = LA.area WHERE LA.id_empresa = '" . $_SESSION["id_empresa"] . "' AND LA.estado = 1 AND LA.anio = '" . $_SESSION["anio_fill"] . "' AND (LA.id_empleado = " . $_SESSION['id_user'] . " OR LA.celula LIKE '%," . $_SESSION['id_user'] . "%' OR LA.celula LIKE '%" . $_SESSION['id_user'] . ",%') ORDER BY Areas.nombre");
    $queryVPLecciones = mysqli_query($connect_clima, "SELECT DISTINCT(LA.id_vp) AS id_vp FROM Lecciones_Aprendidas AS LA INNER JOIN goforagile_admin.Vicepresidencia AS VP ON VP.id = LA.id_vp WHERE LA.id_empresa = '" . $_SESSION["id_empresa"] . "' AND LA.estado = 1 AND LA.anio = '" . $_SESSION["anio_fill"] . "' AND (LA.id_empleado = " . $_SESSION['id_user'] . " OR LA.celula LIKE '%," . $_SESSION['id_user'] . "%' OR LA.celula LIKE '%" . $_SESSION['id_user'] . ",%')  ORDER BY VP.nombre");
    ...
    $queryVPLecciones = mysqli_query($connect_clima, "SELECT DISTINCT(LA.id_vp) AS id_vp FROM Lecciones_Aprendidas AS LA INNER JOIN goforagile_admin.Vicepresidencia AS VP ON VP.id = LA.id_vp WHERE LA.id_empresa = '" . $_SESSION["id_empresa"] . "' AND LA.estado = 1 AND LA.anio = '" . $_SESSION["anio_fill"] . "' AND (LA.celula LIKE '%," . $_SESSION['id_user'] . "%' OR LA.celula LIKE '%" . $_SESSION['id_user'] . ",%')  ORDER BY VP.nombre");
...
if ($_SESSION["role_plataforma"] == 1) {
    $resultado = CargaLeccionesAdminVP($connect_clima, $connect_valentina, $filtro_la, $filtro_kr, $filtro_claves, $filtro_area, $filtro_vp);
} else if ($_SESSION["role_plataforma"] == 2) {
    $resultado = CargaLeccionesLiderVP($connect_clima, $connect_valentina, $filtro_la, $filtro_kr, $filtro_claves, $filtro_area, $filtro_vp, $_SESSION["id_user"]);
} else {
    $resultado = CargaLeccionesColaboradorVP($connect_clima, $connect_valentina, $filtro_la, $filtro_kr, $filtro_claves, $filtro_area, $filtro_vp, $_SESSION["id_user"]);
}
...
$querySM73 = mysqli_query($connect_valentina, "SELECT * FROM Submenu_Empresa WHERE id_empresa = " . $_SESSION["id_empresa"] . " AND estado = 1 AND id_menu = 7 AND id_submenu = 43");
$dataSM73 = mysqli_fetch_array($querySM73);
?>
            ...
            <div class="card-header" style="background-color: #FFFFFF !important;">
                <div class="row">
                    <div class="col-md-12" style="text-align: start !important;">
                        <h4><i class="fas fa-book" id="iconCabecera"></i>&nbsp;&nbsp;|&nbsp;&nbsp;<?php echo $dataSM73["nombre"]; ?></h4>
                    </div>
                </div>
            </div>
        </div>
...
<div class="container-fluid">
    <?php include("views/lecciones_aprendidas/detalle/filtros_vp.php"); ?>
    <?php
                        ...
                        <table border="1" id="lecciones_aprendidas" class="display table" style="width:100%">
                            ...
                            <tbody>
                                ...
                                while ($dataVP = mysqli_fetch_array($queryVP)) {
                                    ...
                                    $id_area_values = array_column($resultado, 'id_vp');
...
<script>
    ...
    $("#bt_la_lecciones_vp").addClass("current-page");
</script>
...
<script>
    ...
    function VerCelula(id,id_user) {
        jQuery.ajax({
                url: api + "ver_celula_vp.php",
                type: 'post',
                data: {
                    id: id,
                    id_user: id_user,
                    url_vp: "?pg=lecciones_aprendidas/detalle/administrar_participantes&id_vp="+id,
                },
            }).done(function(resp) {
                $("#modal_la").modal("show");
