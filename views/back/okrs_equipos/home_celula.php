...
<script>
	...
	$("#bt_okrs_celula").addClass("current-page");
</script>
<style>
	.iconoPA {
...
if ($_POST["filtro_okrs"] != "") {
	echo '<script> window.location.href = "?pg=okrs_equipos/home_celula&page=' . $page . '";</script>';
}
...
if ($_POST["mover_resultado"]) {
	...
	echo '<script> window.location.href = "?pg=okrs_equipos/home_celula&page=' . $page . '&resultado_okr_=' . $_POST["id_registro"] . '";</script>';
}
...
if ($_POST["guardar_iniciativa"] != "") {
	...
	echo '<script> window.location.href = "?pg=okrs_equipos/home_celula&page=' . $page . '&resultado_okr_=' . $_POST["id_resultado"] . '&iniciativa=' . $iniciativa . '";</script>';
}
...
if ($_POST["duplicar_iniciativa"] != "") {
	...
	echo '<script> window.location.href = "?pg=okrs_equipos/home_celula&page=' . $page . '&resultado_okr_=' . $resultado_okr . '&iniciativa=' . $iniciativa . '";</script>';
}
...
if ($_POST["guardar_comentario"] != "") {
	...
	echo '<script> window.location.href = "?pg=okrs_equipos/home_celula&page=' . $page . '&resultado_okr_=' . $_POST["id_resultado"] . '";</script>';
}
...
if ($_POST["editar_comentario_iniciativa"] != "") {
	...
	// echo '<script> window.location.href = "?pg=okrs_equipos/home_celula&resultado_okr_=' . $_POST["id_resultado"] . '";</script>';
}
...
if ($_POST["crear_resultado"] != "") {
	...
	echo '<script> window.location.href = "?pg=okrs_equipos/home_celula&page=' . $page . '&resultado_okr_=' . $resultado_okr . '";</script>';
}
...
if ($_POST["crear_plan_accion"] != "") {
	...
	echo '<script> window.location.href = "?pg=okrs_equipos/home_celula&page=' . $page . '&resultado_okr_=' . $dataPA["id_resultado"] . '&iniciativa=' . $dataPA["id_iniciativa"] . '&pa_iniciativa=' . $dataPA["id_iniciativa"] . '";</script>';
}
...
if ($_POST["duplicar_plan_accion"] != "") {
	...
	echo '<script> window.location.href = "?pg=okrs_equipos/celula&page=' . $page . '&resultado_okr_=' . $_POST["id_resultado"] . '&iniciativa=' . $_POST["id_iniciativa"] . '";</script>';
}
...
//TODOS LOS OKRS DE ESTE USUARIO CON FILTROS APLICADOS
// $ARRAY_OKRS_USUARIO = OkrsUsuario( $dtEmpleado["id"], $dtEmpleado['id_empresa'], $connect_okrs );
// $ARRAY_OKRS_USUARIO = OkrsUsuarioIndividual( $dtEmpleado["id"], $dtEmpleado['id_empresa'], $connect_okrs );
$ARRAY_OKRS_USUARIO = OkrsUsuarioCelula($dtEmpleado["id"], $dtEmpleado['id_empresa'], $connect_okrs, $filtro_periodo);
$ARRAY_OKRS_USUARIO_PAGINACION = OkrsUsuarioCelulaPaginacion($dtEmpleado["id"], $dtEmpleado['id_empresa'], $connect_okrs, $inicio, $registros_por_pagina, $filtro_periodo);
...
$querySM83 = mysqli_query($connect_valentina, "SELECT * FROM Submenu_Empresa WHERE id_empresa = " . $_SESSION["id_empresa"] . " AND estado = 1 AND id_menu = 8 AND id_submenu = 47");
$dataSM83 = mysqli_fetch_array($querySM83);
	...
	<div class="col-md-12">
		<div class="card">
			<div class="card-header">
				<div class="row">
					<div class="col-md-12">
						<h4><i class="fas fa-bullseye" id="iconCabecera"></i>&nbsp;&nbsp;|&nbsp;&nbsp;<?php echo $dataSM83["nombre"]; ?></h4>
					</div>
				</div>
			</div>
		</div>
...
<div class="container-fluid" id="area_trabajo">
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				<div class="card-body">
					<?php include("views/okrs_equipos/componentes/filtros_celula.php"); ?>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="row">
		<div class="col-md-12">
			...
			if ($page > 1) {
				echo '<li class="page-item"><a class="page-link" href="?pg=okrs_equipos/home_celula&page=' . ($page - 1) . '">Anterior</a></li>';
			}

			// Enlaces numéricos
			for ($i = 1; $i <= $total_paginas; $i++) {
				$activo = ($i == $page) ? 'active' : '';
				echo '<li class="page-item ' . $activo . '"><a class="page-link" href="?pg=okrs_equipos/home_celula&page=' . $i . '">' . $i . '</a></li>';
			}

			// Enlace a la página siguiente
			if ($page < $total_paginas) {
				echo '<li class="page-item"><a class="page-link" href="?pg=okrs_equipos/home_celula&page=' . ($page + 1) . '">Siguiente</a></li>';
			}
...
$contOkrCelula = count($ARRAY_OKRS_USUARIO);
...
$promedioOkr1 = round((($sumPromedio1 / $contOkrCelula) * 100), 2);
...
$promedioOkr2 = round((($sumPromedio2 / $contOkrCelula) * 100), 2);
...
$promedioOkr3 = round((($sumPromedio3 / $contOkrCelula) * 100), 2);
...
$promedioOkr4 = round((($sumPromedio4 / $contOkrCelula) * 100), 2);
...
$promedioOkr5 = round((($sumPromedio5 / $contOkrCelula) * 100), 2);
