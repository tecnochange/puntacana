...
if ($_POST["filtro_kpis"] != "") {
    echo '<script> window.location.href = "?pg=kpis_pc/kpis_colaborador&page=1";</script>';
}
...
if ($_POST["guardar_semestral"] != "") {
    ...
    echo '<script> window.location.href = "?pg=kpis_pc/kpis_colaborador&kpi_=' . $_POST["id_kpi"] . '' . $pagina . '";</script>';
}
...
$querySM65 = mysqli_query($connect_valentina, "SELECT * FROM Submenu_Empresa WHERE id_empresa = " . $_SESSION["id_empresa"] . " AND estado = 1 AND id_menu = 6 AND id_submenu = 37");
$dataSM65 = mysqli_fetch_array($querySM65);
...
<script>
    ...
    $("#bt_desempenio_mis_kpis").addClass("current-page");
</script>
            ...
            <div class="card-header" style="background-color: #FFFFFF !important;">
                <div class="row">
                    <div class="col-md-12" style="text-align: start !important;">
                        <h4 style="font-family: Lato-Black !important;color: #365189 !important;font-size: 1.5rem !important;"><i class="fas fa-chart-line" id="iconCabecera"></i>&nbsp;&nbsp;|&nbsp;&nbsp;<?php echo $dataSM65["nombre"]; ?></h4>
                    </div>
                </div>
            </div>
        </div>
...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <?php include("views/kpis_pc/detalle/filtro_kpis_colaborador.php"); ?>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div id="accordionIconsKpis" class="accordion-icons" role="tablist">
                    ...
                    WHERE K.id_empresa = " . $_SESSION['id_empresa'] . " AND K.anio = " . $_SESSION['anio_fill'] . " AND KC.id_colaborador = " . $_SESSION['id_user'] . " $filtro ORDER BY K.indicador ASC");
                    ...
                    INNER JOIN Kpis_Colaborador KC ON KC.id_kpi = K.id
                    INNER JOIN Frecuencia_Kpis FKP ON FKP.id_kpi = K.id
                    WHERE K.id_empresa = " . $_SESSION['id_empresa'] . " AND K.anio = " . $_SESSION['anio_fill'] . " AND KC.id_colaborador = " . $_SESSION['id_user'] . " $filtro ORDER BY K.indicador ASC LIMIT $inicio, $registros_por_pagina");
                    $total_paginas = ceil($total_registros / $registros_por_pagina);
                    // echo mysqli_num_rows($queryKpis);
                    if (mysqli_num_rows($queryKpis) == 0 || mysqli_num_rows($queryKpisP) == 0) {
                        echo '
                        <div class="alert alert-primary" role="alert">
                            No se tiene KPIS asignados.
                        </div>
                        ';
                    }
                    $sumSeguimiento = $sumAvance = $contKpi = $sumAvanceC = $contKpiC = $contKpiT = $contKpiE = $sumAvanceE = $sumAvanceT = 0;
...
$photo = $dataEmpleado["foto"];
                                        ...
                                        <div class="row" style="align-items: center;">
                                            ...
                                            if ($_SESSION['role_plataforma'] == 1 || $permisoEditKPI == true || $permisoMetaKPI == true) { ?>
                                                <div class="col-md-9" style="text-align:end;">
                                                    <button type="button" class="btn btn-outline-dark btn-sm bt_editar" data-bs-toggle="tooltip" title="Editar KPI" onclick="location.href='<?php echo $url; ?>?pg=kpis_pc/detalle/editar&id_kpi=<?php echo $dataKpis['id']; ?>&id_frecuencia=<?php echo $dataKpis['obj_meses']; ?>&page=<?php echo $_GET['page']; ?>&menu=kpis_colaborador'">
                                                        <i class="fas fa-edit" style="font-size: 15px !important;"></i>
                                                    </button>

                                                    <?php if ($_SESSION['role_plataforma'] == 1 || $permisoEditIntegrantes == true) { ?>
                                                        <button type="button" class="btn btn-outline-dark btn-sm bt_editar" data-bs-toggle="tooltip" title="Editar KPI" onclick="location.href='<?php echo $url; ?>?pg=kpis_pc/detalle/asignar_colaborador_lider&id_kpi=<?php echo $dataKpis['id']; ?>&id_frecuencia=<?php echo $dataKpis['obj_meses']; ?>&page=<?php echo $_GET['page']; ?>&menu=kpis_colaborador'">
                    ...
                    include("views/kpis_pc/desempenio_kpis_colaborador.php");

                    $avanceGeneral = $sumAvanceC / $contKpiC;
    ...
    <div class="row">
        <div class="col-md-12 table-responsive">
            ...
            if ($page > 1) {
                echo '<li class="page-item"><a class="page-link" href="?pg=kpis_pc/kpis_colaborador&page=' . ($page - 1) . '">Anterior</a></li>';
            }

            // Enlaces numéricos
            for ($i = 1; $i <= $total_paginas; $i++) {
                $activo = ($i == $page) ? 'active' : '';
                echo '<li class="page-item ' . $activo . '"><a class="page-link" href="?pg=kpis_pc/kpis_colaborador&page=' . $i . '">' . $i . '</a></li>';
            }

            // Enlace a la página siguiente
            if ($page < $total_paginas) {
                echo '<li class="page-item"><a class="page-link" href="?pg=kpis_pc/kpis_colaborador&page=' . ($page + 1) . '">Siguiente</a></li>';
            }
...
<script>
    ...
    function GuardarAvances(id) {
        window.location = "?pg=kpis_pc/kpis_colaborador&kpi_=" + id;
    }
