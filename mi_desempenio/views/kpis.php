...
while ($dataKpis = mysqli_fetch_array($queryKpis)) {
    $fechaHoy = date("Y-m");
    $mesHoy = date("m");
    ...
    if ($sumSeguimiento != "") {
        ...
        if ($dataKpis["tipo_calculo"] == 2) {
            ...
            $progresoAvance = CalculoProgresoDesMes($metaFiltro, $sumSeguimiento);
            // }
        }
        ...
        if ($metaFiltro == 0 && $sumSeguimiento == 0) {
            $progresoF = $progresoAvance = 100;
        }
    } else {
        $progresoF = $progresoAvance = 0;
    }

    if ($dataKpis["tipo_calculo"] == 2 && $dataKpis["unidad_medida"] != 4) {

        if ($metaFiltro >= 0 && ($sumSeguimiento === null || $sumSeguimiento === '')) {
            $progresoF = $progresoAvance = 0; // Meta es 0 y sumSeguimiento vacío o nulo
        } elseif ($metaFiltro >= 0 && $sumSeguimiento == 0) {
            $progresoF = $progresoAvance = 100; // Meta y sumSeguimiento son iguales a 0                            
        } elseif ($metaFiltro == 0 && $sumSeguimiento > 0) {
            $progresoF = $progresoAvance = max(0, 100 - ($sumSeguimiento * 10));
        } elseif ($metaFiltro == 0 && $sumSeguimiento < 0) {
            $progresoF = $progresoAvance = 100;
        } elseif ($metaFiltro > 0 && $sumSeguimiento < 0) {
            $progresoF = $progresoAvance = max(0, 100 - ($sumSeguimiento * 10));
    ...
    if ($dataKpis["tipo_calculo"] == 3 && $dataKpis["unidad_medida"] != 4) {
        ...
        if ($metaFiltro >= 0 && ($sumSeguimiento === null || $sumSeguimiento === '')) {
            ...
            $progresoF = $progresoAvance = 100; // Meta y sumSeguimiento son 0 (no progreso, pero la meta está cumplida)
        } elseif ($metaFiltro == 0 && $sumSeguimiento > 0) {
            $progresoF = $progresoAvance = 0; // Meta 0, pero seguimiento positivo (no se cumple la meta)
        } elseif ($metaFiltro == 0 && $sumSeguimiento < 0) {
            $progresoF = $progresoAvance = 100; // Meta 0, seguimiento negativo (se cumple la meta en este caso)
        } elseif ($metaFiltro > 0 && $sumSeguimiento < 0) {
            $progresoF = $progresoAvance = 0; // Meta positiva, pero seguimiento negativo (no se cumple la meta)
        } elseif (abs($sumSeguimiento) <= $metaFiltro) {
            $progresoF = $progresoAvance = 100; // Seguimiento dentro de los límites de la meta (cumple la meta)
        } elseif (abs($sumSeguimiento) > $metaFiltro) {
            $progresoF = $progresoAvance = 0; // Seguimiento mayor que la meta (no se cumple la meta)
        }
